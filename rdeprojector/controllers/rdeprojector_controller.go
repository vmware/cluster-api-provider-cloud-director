...
func (r *RDEProjectorReconciler) reconcileNormal(ctx context.Context, rdeProjector *rdeprojectorv1alpha1.RDEProjector) (ctrl.Result, error) {
	...
	entity := rde.Entity
	capiYaml, ok := entity["spec"].(map[string]interface{})["capiYaml"]
	if !ok {
		return ctrl.Result{}, errors.Wrapf(nil, "Missing CAPI yaml in the RDE spec of id [%s]", rde.Id)
	}
	capiYamlStr, ok := capiYaml.(string)
	if !ok {
		return ctrl.Result{}, errors.Wrapf(nil, "CapiYaml specified in the RDE [%s] is not of type String", rde.Id)
	}
	log.Info("Retrieved Capi Yaml", "capiYaml", capiYamlStr)
	...
	yamlReader := k8syaml.NewYAMLReader(bufio.NewReader(bytes.NewReader([]byte(fmt.Sprintf("%v", capiYamlStr)))))
	...
	for err == nil {
		...
		if err == io.EOF {
			...
			return ctrl.Result{}, errors.Wrapf(err, "Error reading object from the CAPI yaml of rdeId [%s]; yaml object: [%v]", rdeProjector.Spec.RDEId, string(yamlBytes))
		}
		...
		if err != nil {
			return ctrl.Result{}, errors.Wrapf(err, "Error decoding CAPI object from the CAPI yaml of rdeId [%s] into unstructured format; capi yaml object : [%v] ", rdeProjector.Spec.RDEId, string(yamlBytes))
		}
		force := true
		...
		log.Info("Applied the patch of the object", "object", string(yamlBytes))
		...
		//})
		//if err != nil {
		//	return ctrl.Result{}, errors.Wrapf(err, "Error patching capi object [%v] via SSA; rdeId: [%s]", obj, rdeProjector.Spec.RDEId)
		//}
	}
