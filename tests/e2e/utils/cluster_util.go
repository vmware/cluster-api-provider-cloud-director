...
func ConstructCAPVCDTestClient(ctx context.Context, cs *kubernetes.Clientset, capiYaml, clusterNameSpace, clusterName, rdeId string, getVdcClient bool) (*testingsdk.TestClient, error) {
	...
	vcdCluster, err := GetVCDClusterFromCluster(ctx, cs, clusterNameSpace, clusterName)

	userOrgName, userName := GetUserOrgAndName(ctx, vcdCluster.Status.Org, username)

	testClient, err := NewTestClient(vcdCluster.Status.Site, vcdCluster.Status.Org, userOrgName, userName, username, refreshToken, rdeId, getVdcClient)
	if err != nil {
...
func CreateWorkloadClusterResources(ctx context.Context, workloadClientSet *kubernetes.Clientset, capiYaml, namespace, rdeID string, vcdcluster *v1beta2.VCDCluster) error {
